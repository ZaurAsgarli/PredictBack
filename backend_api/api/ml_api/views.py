...
from ml_service.training.logs import log_model_output
from ml_service.training.model_loader import predict_trade_risk
from ml_service.training.services.db_storage_service import save_trade_risk_prediction
from ml_service.training.services.exposure_service import (
    get_user_exposure,
    get_market_risk,
...
from ml_service.training.services.manipulation_service import (
    get_market_manipulation_analysis,
    get_all_markets_manipulation_analysis,
    get_user_manipulation_risk,
)
from ml_service.training.services.mhews_service import (
    get_platform_health_status,
    get_health_dashboard_data,
)
from ml_service.training.services.token_behavior_service import (
    forecast_token_behavior_for_market,
)
from ml_service.training.logs import log_token_behavior_prediction
from ml_service.training.model_loader import TokenBehaviorModelError
...
class TokenBehaviorForecastView(APIView):
    """
    GET /api/ml/token-behavior/market/<market_id>/
    
    Returns token behavior forecasts (price movement predictions) for a market.
    
    Query parameters:
        - window_hours (int, default=24): Time window in hours for analysis
        - start_time (ISO datetime, optional): Start time for analysis window
        - end_time (ISO datetime, optional): End time for analysis window
    ...
    def get(self, request: Request, market_id: int) -> Response:
        try:
            ...
            # Get forecast
            result = forecast_token_behavior_for_market(
                market_id=market_id,
            ...
            for pred in result.get("predictions", []):
                try:
                    ...
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.warning(f"Failed to log token behavior prediction: {e}")
            
            return Response(result, status=status.HTTP_200_OK)
            
        except TokenBehaviorModelError as e:
            return Response(
                {"error": "Model error", "detail": str(e)},
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
            )
        except ValueError as e:
            return Response(
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.exception("Token behavior forecast failed")
            return Response(
                {"error": "Forecast failed", "detail": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
