...
class Command(BaseCommand):
    ...
    def seed_security_logs(self, users, count):
        ...
        for i in range(count):
            ...
            log_method = getattr(security_logger, severity.lower() if severity != 'CRITICAL' else 'critical')
    ...
    def seed_ml_logs(self, users, trades, high_risk_count):
        ...
        for user in high_risk_users:
            ...
            # Use 'score' field (float between -1 and 1, higher = more anomalous)
            # For high risk, we want scores close to 1
            score_value = random.uniform(0.80, 0.99)
            ...
            prediction = TradeRiskPrediction(
                ...
                score=score_value,
                label=-1,  # -1 = anomaly
                risk_level='CRITICAL' if score_value > 0.95 else 'HIGH',
                model_version='v2.1-suspicious-detector',
                amount_staked=Decimal(str(random.uniform(1000, 50000))).quantize(Decimal('0.01')) if trade else None,
                user_total_trades=random.randint(50, 200),
            )
            predictions.append(prediction)
            
            ml_logger.warning(
                f'[USER:{user.id}] HIGH_RISK_DETECTED score={score_value:.2f} '
        ...
        for user in normal_users:
            ...
            # Normal users have lower scores
            score_value = random.uniform(-0.5, 0.5)
            risk_level = 'LOW' if score_value < 0.3 else 'MEDIUM'
            
            prediction = TradeRiskPrediction(
                user=user,
                trade=trade,
                score=score_value,
                label=1 if score_value < 0.5 else -1,  # 1 = normal
                risk_level=risk_level,
                model_version='v2.1-suspicious-detector',
            )
            predictions.append(prediction)
            
            ml_logger.info(f'[USER:{user.id}] RISK_ASSESSED score={score_value:.2f} level={risk_level}')
