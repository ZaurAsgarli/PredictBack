...
def calculate_market_health(
    ...
    model_path_stress: Optional[str] = None,
    model_path_risk: Optional[str] = None,
) -> pd.DataFrame:
    """
    ...
    Can use trained ML models if available, otherwise falls back to heuristic aggregation.
    ...
    Args:
        ...
        use_ml_model: If True, use trained ML models (if available)
        model_path_stress: Path to stress level model
        model_path_risk: Path to risk index model
    ...
    # Try to use ML models if requested
    ml_models_available = False
    if use_ml_model:
        try:
            from ml_service.training.notebooks.model_5_trainer import predict_health_ml
            from pathlib import Path
            
            if model_path_stress is None:
                model_path_stress = Path(__file__).parent.parent / 'models' / 'model5_health_platform_stress_level.pkl'
            else:
                model_path_stress = Path(model_path_stress)
            
            if model_path_risk is None:
                model_path_risk = Path(__file__).parent.parent / 'models' / 'model5_health_systemic_risk_index.pkl'
            else:
                model_path_risk = Path(model_path_risk)
            
            if model_path_stress.exists() and model_path_risk.exists():
                ml_models_available = True
        except Exception as e:
            # Fall back to heuristic if ML models not available
            pass
    
    # Calculate composite metrics (ML model or heuristic)
    if ml_models_available:
        try:
            ml_stress = predict_health_ml(
                df_model1, df_model2, df_model3, df_model4,
                target='platform_stress_level',
                model_path=model_path_stress
            )
            platform_stress_level = ml_stress['platform_stress_level']
            
            ml_risk = predict_health_ml(
                df_model1, df_model2, df_model3, df_model4,
                target='systemic_risk_index',
                model_path=model_path_risk
            )
            systemic_risk_index = ml_risk['systemic_risk_index']
        except Exception as e:
