...
def detect_market_manipulation(
    ...
    Can use trained ML model if available, otherwise falls back to heuristic detection.
    ...
    Args:
        ...
        use_ml_model: If True, use trained ML model (if available)
        model_path: Path to trained model (None = use default location)
    ...
    # Try to use ML model if requested
    ml_model_available = False
    if use_ml_model:
        try:
            from ml_service.training.notebooks.model_4_trainer import load_manipulation_model, predict_manipulation_ml
    ...
    for mkt_id in df['market_id'].unique():
        ...
        # 6. Get manipulation score (ML model or heuristic)
        if ml_model_available:
            try:
                # Use ML model for market-level prediction
                ml_result = predict_manipulation_ml(df_trades, mkt_id, None, model_path)
                base_manipulation_score = ml_result['manipulation_score']
            except Exception as e:
                # Fall back to heuristic
                max_clique_score = max([cs['score'] for cs in clique_scores.values()]) if clique_scores else 0.0
                base_manipulation_score = (
                    0.4 * max_clique_score +
                    0.35 * pump_dump_score +
                    0.25 * wash_score
                )
        else:
            # Use heuristic combination
            max_clique_score = max([cs['score'] for cs in clique_scores.values()]) if clique_scores else 0.0
            base_manipulation_score = (
                0.4 * max_clique_score +
                0.35 * pump_dump_score +
        ...
        if base_manipulation_score >= 0.8:
            risk_level = 'CRITICAL'
        elif base_manipulation_score >= 0.6:
            risk_level = 'HIGH'
        elif base_manipulation_score >= 0.4:
            risk_level = 'MEDIUM'
        else:
            risk_level = 'LOW'
        ...
        for user_id in market_users:
            ...
            # User-specific manipulation score
            if ml_model_available:
                try:
                    # Use ML model for user-specific prediction
                    ml_result = predict_manipulation_ml(df_trades, mkt_id, user_id, model_path)
                    user_manipulation_score = ml_result['manipulation_score']
                except:
                    # Fall back to base score with clique adjustment
                    user_manipulation_score = base_manipulation_score
                ...
                # Use heuristic: base score with clique adjustment
                user_manipulation_score = base_manipulation_score
                if user_clique_id is not None:
                    user_manipulation_score = max(
                        user_manipulation_score,
